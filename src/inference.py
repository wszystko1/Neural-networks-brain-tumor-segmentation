...
def infer(device: torch.device, run_number: int, brain_numbers: list):
    ...
    checkpoint = torch.load(f"results/unet_checkpoint_{run_number}.pth", map_location=device)
    ...
    plt.tight_layout()
    plt.savefig(f"results/inference_results_{run_number}.png", dpi=150, bbox_inches="tight")
    plt.show()
